...
QRELS_FILE = "query_eval/qrels_ate_with.txt"
QUERY_URL_NEW = "http://localhost:8983/solr/wines/select?indent=true&q.op=OR&q=note%3A%22ate%20with%22~20"

# Read qrels to extract relevant documents
relevant = list(map(lambda el: el.strip(), open(QRELS_FILE).readlines()))
# Get query results from Solr instance
results = requests.get(QUERY_URL_NEW).json()['response']['docs']
